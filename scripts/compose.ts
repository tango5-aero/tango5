import { circle, type Units } from '@turf/turf';
import type { Feature, FeatureCollection, GeoJsonProperties, Geometry } from 'geojson';

import { CornerToRectangle, type LatLng, LocalGeometry, type Rectangle, RectangleToPolygon, SegmentToLine } from './geometry.ts';
import type { Flight, Pcd } from './flight.ts';
...
function composeMapElements(flights: Flight[]): Feature[] {
    const features = newFeatureCollector();

    const avg_lat = flights.map((flight) => flight.points4d.synced.latDeg).reduce((acc, cur) => (acc += cur), 0) / flights.length;
    const avg_lon = flights.map((flight) => flight.points4d.synced.lonDeg).reduce((acc, cur) => (acc += cur), 0) / flights.length;

    const geometry = new LocalGeometry({ lat: avg_lat, lng: avg_lon });
